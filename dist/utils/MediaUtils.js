...
export class MediaUtils {
     ...
     */
    static async extractVideoMetadata(file) {
        return new Promise((resolve) => {
     ...
     */
    static validateMedia(file, metadata, rules) {
        const errors = [];
        // File validation
        if (file.size <= 0) {
     ...
     */
    static getAspectRatioName(aspectRatio) {
        const ratios = {
            '1.00': '1:1',
            '1.33': '4:3',
            '1.78': '16:9',
     ...
     */
    static calculateBitRate(fileSize, duration) {
        if (duration <= 0)
            return 0;
     ...
     */
    static async generateImageThumbnail(file, options) {
        if (typeof window === 'undefined' || !file.data) {
            return null;
        }
        return new Promise((resolve) => {
     ...
     */
    static async hasTransparency(file) {
     ...
     */
    static calculateQualityScore(metadata, fileSize) {
     ...
     */
    static calculateSizeScore(fileSize, metadata) {
        if (!metadata.width || !metadata.height)
            return 0;
        const pixels = metadata.width * metadata.height;
        ...
        if (bytesPerPixel >= 1 && bytesPerPixel <= 4)
            return 10; // Good compression
        if (bytesPerPixel >= 0.5 && bytesPerPixel < 1)
            return 5; // High compression
        if (bytesPerPixel > 4)
            return -5; // Oversized
        if (bytesPerPixel < 0.5)
            return -10; // Over-compressed
        return 0;
    }
}
